...
def get(package_json_including_folder, npm_licence_checker_binary):
    ...
    def gen():
        ...
        for name, details in jsDict.iteritems():
            yield Material(''.join(name.split("@")[:-1]), details["licenses"],
                           format_javascript,
                           details.get("repository", details.get("url", "")))
